...
Envia uma mensagem com um botão interativo. O endpoint `send_wbuttons` é uma abreviação para **Send With Buttons** (*Enviar com Botão*). Este sistema é **totalmente stateless**: as instruções do que acontece no clique estão embutidas no ID do botão de forma criptografada.
